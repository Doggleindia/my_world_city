...
/**
 * Send the code that lets an admin set a new console password.
 * Same envelope as the login code, different wording so it can't be confused.
 */
export async function sendAdminResetEmail(to, code, minutes = 5) {
  const result = await sendEmail({
    to,
    subject: `${code} is your My World City admin password reset code`,
    text:
      `Use this code to set a new admin password: ${code}` +
    ...
    html: otpEmailHtml(code, minutes, {
      ...
      intro: 'Enter the code below on the reset screen, then choose a new password.',
      footer: 'If you did not ask to reset your password, ignore this email — nothing has changed.',
    }),
  })
...
function otpEmailHtml(code, minutes, copy = {}) {
  const title = copy.title || 'Verify your email'
  const intro = copy.intro || `Use the code below to finish signing in. It is valid for ${minutes} minutes.`
  const footer =
    copy.footer ||
    'Never share this code with anyone — our team will never ask you for it. If you did not request this code, you can safely ignore this email.'
  return `<!doctype html>
<html>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e2e8f0;">
            ...
            <tr>
              <td style="padding:30px 28px 8px;">
                <h1 style="margin:0;font-size:20px;font-weight:800;color:#0f1d33;">${title}</h1>
                <p style="margin:10px 0 0;font-size:14px;line-height:22px;color:#64748b;">${intro}</p>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td style="padding:0 28px 28px;">
                <p style="margin:0;font-size:13px;line-height:21px;color:#94a3b8;">${footer}</p>
              </td>
            </tr>
          </table>
