...
export function handler(fn) {
  return async (req, ctx) => {
    try {
      ...
      // Mongo unique-index violation — the visitor gets a clear message rather
      // than a bare 500, and the field name is kept out of the response.
      if (e?.code === 11000) {
        console.error('[api] duplicate key:', e.keyValue || e.message)
        return fail('That value is already registered on another account.', 409)
      }
